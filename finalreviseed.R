...
df = read.csv("C:/Users/chelsie/desktop/auresults.csv")
df = df[,-1]
...
#//////////////////////////////////////////////////////////////
m_color=c("red","green","blue","cyan","magenta","yellow","black","brown")

graphics.off()
data1 = df[2:3]
X_range = range(data1[,1])
Y_range = range(data1[,2])


png(file="C:/Users/chelsie/desktop/au 7 k.png",  width=2000, 800)
...
for (n in 1:k) {
  ...
  points(cluster_data[,1], cluster_data[,2],pch=16,col=m_color[n])
  mu <- colMeans(cluster_data)
  Sigma <- cov(cluster_data)
  addEllipse(mu, Sigma, p.interval = p, col = m_color[n], lty = 2, lwd=2)
  
  label = paste("Cluster", n)
  legend_string <- c(legend_string, label) 
  color_string <- c(color_string, m_color[n]) 
}

legend(1,Y_range[2]-10, legend = legend_string, col=color_string, pch=1,lty=1, merge=T)#, trace=T)
